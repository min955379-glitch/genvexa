...
function AuthModal({ onClose, onSubmit }) {
  ...
  return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><div className="auth-modal"><button className="modal-close" onClick={onClose}><X size={18} /></button><div className="auth-logo"><span className="brand-mark"><Crown size={19} fill="currentColor" /></span></div><span className="section-kicker">WELCOME TO GENVEXA</span><h2>Keep your ideas close.</h2><p>Sign in to sync favorites, history, and credits across devices.</p><form onSubmit={submit}><label>Email<input type="email" required value={email} onChange={event => setEmail(event.target.value)} placeholder="you@example.com" /></label><label>Password<input type="password" required value={password} onChange={event => setPassword(event.target.value)} placeholder="••••••••" /></label>{error && <div className="form-error">{error}</div>}<button className="button-primary button-wide" disabled={loading}>{loading ? 'Signing in...' : 'Continue with email'} <ArrowUpRight size={15} /></button></form><div className="auth-divider"><span>Demo account</span></div><button className="demo-login" onClick={() => { setEmail('admin@genvexa.test'); setPassword('admin123'); }}>Use admin@genvexa.test · admin123</button><small className="auth-footnote">By continuing, you agree to our Terms and Privacy Policy.</small></div></div>;
}
...
function AdminGate({ onLogin, navigate }) {
  const [email, setEmail] = useState('admin@genvexa.test');
  ...
  return <div className="admin-gate"><div className="admin-gate-brand"><span className="brand-mark"><Crown size={18} fill="currentColor" /></span><strong>Genvexa</strong><span>Control room</span></div><div className="admin-gate-card"><div className="admin-lock"><ShieldCheck size={22} /></div><span className="section-kicker">RESTRICTED WORKSPACE</span><h1>Sign in to the admin portal.</h1><p>Manage prompts, creators, reviews, and platform activity from one place.</p><form onSubmit={submit}><label>Email<input value={email} onChange={event => setEmail(event.target.value)} type="email" required /></label><label>Password<input value={password} onChange={event => setPassword(event.target.value)} type="password" required /></label>{error && <div className="form-error">{error}</div>}<button className="button-primary button-wide" disabled={loading}>{loading ? 'Checking access...' : 'Enter control room'} <ArrowUpRight size={15} /></button></form><button className="back-to-site" onClick={() => navigate('/') }><ArrowLeft size={15} /> Back to gallery</button><small>Demo: admin@genvexa.test / admin123</small></div></div>;
}
...
function AdminPromptModal({ onClose, onSave }) { const [title, setTitle] = useState(''); const [prompt, setPrompt] = useState(''); const [category, setCategory] = useState('Ads & Product'); const [model, setModel] = useState('GPT Image'); const submit = event => { event.preventDefault(); onSave({ title, prompt, category, model, excerpt: prompt.slice(0, 110) }); }; return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><div className="admin-prompt-modal"><button className="modal-close" onClick={onClose}><X size={18} /></button><span className="section-kicker">CONTENT LIBRARY</span><h2>Create a prompt</h2><p>Add a new prompt directly to the gallery.</p><form onSubmit={submit}><label>Title<input required value={title} onChange={event => setTitle(event.target.value)} placeholder="Prompt title" /></label><label>Prompt<textarea required value={prompt} onChange={event => setPrompt(event.target.value)} rows="7" placeholder="Write the prompt..." /></label><div className="form-grid"><label>Category<select value={category} onChange={event => setCategory(event.target.value)}>{categories.filter(item => item.name !== 'All').map(item => <option key={item.name}>{item.name}</option>)}</select></label><label>Model<select value={model} onChange={event => setModel(event.target.value)}>{modelOptions.map(option => <option key={option}>{option}</option>)}</select></label></div><div className="admin-modal-actions"><button type="button" className="button-secondary" onClick={onClose}>Cancel</button><button className="button-primary">Create prompt <ArrowUpRight size={15} /></button></div></form></div></div>; }
